...
void startPms()
{
  ...
  if (pms.set_sleep(false))
  {
    ...
    // The sensor always wakes in active mode, command to passive mode
    Serial.println("StartPms: Turning on Pms5003::REPORT_PASSIVE reporting mode");
    if (!pms.set_data_reporting_mode(Pms5003::REPORT_PASSIVE))
    {
      Serial.println("StartPms: Pms5003::set_data_reporting_mode(Pms5003::REPORT_ACTIVE) failed");
    }
  }
